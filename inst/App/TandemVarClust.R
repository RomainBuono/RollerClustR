  ...
  private = list(
    ...
    predict_variable = function(new_var, var_name = "new_var") {
      ...
      if (is.factor(new_var) || is.character(new_var)) {
        ...
        levels_var <- levels(new_var)
        
        for (level in levels_var) {
          indicator <- as.integer(new_var == level)
          col_name <- paste0(var_name, ".", level)
          disjunctive_list[[col_name]] <- indicator
        }
        
      } else if (is.numeric(new_var)) {
        if (private$FScale) {
          new_var_scaled <- scale(new_var)[, 1]
        } else {
          new_var_scaled <- new_var
        }
        
        breaks <- quantile(new_var_scaled, 
                           probs = seq(0, 1, length.out = private$FNBins + 1),
                           na.rm = TRUE)
        breaks <- unique(breaks)
        
        if (length(breaks) <= 1) {
          stop("Variable has zero or very low variance. Cannot predict.")
        }
        
        discretized <- cut(new_var_scaled, breaks = breaks, 
        ...
        stop("new_var must be numeric, factor, or character")
      }
      
      Z_new <- as.data.frame(disjunctive_list)
      
      if (ncol(Z_new) == 0) {
        stop("The disjunctive table for new variable is empty.")
      }
      ...
      # Validation : vérifier que n_factors_to_use ne dépasse pas les dimensions disponibles
      ...
      # Projection des nouvelles modalités comme éléments supplémentaires
      # On ne peut PAS utiliser D_col_new_sqrt %*% FSVDv car dimensions incompatibles
      # À la place, on calcule les coordonnées comme moyenne pondérée des observations
      
      n_modalities_new <- ncol(Z_new)
      factorial_coords_new <- matrix(0, nrow = n_modalities_new, ncol = n_factors_to_use)
      rownames(factorial_coords_new) <- colnames(Z_new)
      colnames(factorial_coords_new) <- paste0("Dim", 1:n_factors_to_use)
      
      # Calculer les coordonnées factorielles des observations d'entraînement UNE SEULE FOIS
      # Ces coordonnées sont : D_row_sqrt %*% Z_centered %*% D_col_sqrt %*% v
      # = (1/sqrt(n)) * Z_centered %*% FFactorialCoords
      ...
      row_coords <- (1/sqrt(n)) * Z_train_centered %*% private$FFactorialCoords[, 1:n_factors_to_use, drop = FALSE]
      
      # Pour chaque modalité de la nouvelle variable
      for (j in 1:n_modalities_new) {
        # Observations ayant cette modalité
        obs_with_modality <- which(Z_new[, j] == 1)
        n_obs_modality <- length(obs_with_modality)
        
        if (n_obs_modality > 0) {
          # Barycentre des observations ayant la modalité j
          factorial_coords_new[j, ] <- colMeans(row_coords[obs_with_modality, , drop = FALSE])
        }
      }
      
      n_modalities_new <- nrow(factorial_coords_new)
      distances <- matrix(NA, nrow = n_modalities_new, ncol = private$FNbGroupes)
      rownames(distances) <- rownames(factorial_coords_new)
      colnames(distances) <- paste0("Cluster_", 1:private$FNbGroupes)
      
      for (i in 1:n_modalities_new) {
        for (k in 1:private$FNbGroupes) {
          distances[i, k] <- sqrt(sum((factorial_coords_new[i, ] - 
                                         private$FClusterCenters[k, ])^2))
        }
      }
      
      modality_clusters <- apply(distances, 1, which.min)
      
      cluster_counts <- table(modality_clusters)
      variable_cluster <- as.integer(names(cluster_counts)[which.max(cluster_counts)])
      
      result <- list(
        cluster = variable_cluster,
        modality_clusters = modality_clusters,
        distances = distances,
        factorial_coords = factorial_coords_new,
        cluster_centers = private$FClusterCenters,
        n_modalities = n_modalities_new
      )
      
      class(result) <- c("TandemVarClust_predict_variable", "list")
      
      message("\nVariable '", var_name, "' assigned to Cluster ", variable_cluster)
      message("  Number of modalities: ", n_modalities_new)
      message("  Modality assignments: ", paste(modality_clusters, collapse = ", "))
